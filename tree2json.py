...
data = open_cache(CACHE_FILENAME)
list_of_state = list(data.keys())
tree_list = [Tree(city) for city in list_of_state]
for idx,item in enumerate(data.values()):
    for item_dict in item:
        tree_list[idx].insert_node(item_dict)



# save data to tree json
cities_dict={}
for idx,tree in enumerate(tree_list):
    cities_dict[list_of_state[idx]]=tree.tree2json()
save_cache(cities_dict)
